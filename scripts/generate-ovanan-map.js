...
const AMATEUR = `GENUINELY BAD DRAWING — HIGHEST PRIORITY: Not merely simple — actively embarrassing draftsmanship. Lines overshoot and stop short, outlines double-traced shakily, ink blots and smudges, colour scribbled outside borders, visible eraser ghosts, one dock longer than the other for no reason, a house roof floating off its walls. The cartographer is trying hard but has no talent — every mark looks hesitant, wrong, or corrected mid-stroke. Homemade panto poster on stained parchment. Camp charm through incompetence, NOT through clean minimalism. NEVER precise, NEVER slick, NEVER professionally shaded.`

const CARTOON = `CARTOONISH, VARIED, AND BADLY EXECUTED: Thick wobbly marker outlines — uneven weight, corners that miss. Trees = MANY different bad cartoon trees all over the forest — vary size wildly (tiny stub trees, medium lollipops, a few comically oversized trees), vary shape (round blob, triangle pine scribble, lopsided broccoli, bent trunk leaning wrong way, two trunks on one canopy, canopy floating above the stick). No two trees alike; each drawn badly in its own way. NO tree shading, NO fine branches, flat green blobs on brown sticks. Rocks along west shore = pebbles and boulders of different sizes — some huge, some tiny, overlapping messily, childish squiggles inside, one rock drawn as an obvious potato. Houses = four or five cottages each a different bad design — mismatched roof angles, doors too big, chimneys crooked, one house noticeably worse than the others. Docks = two bent piers, different lengths and angles. Water = flat teal with a few lazy wavy marks of different sizes. Visual variety everywhere but unified by incompetent execution.`
...
const GEOGRAPHY = `HIGHEST PRIORITY — MATCH THE INPUT SATELLITE LAYOUT: Transform the reference photo into a cartoonish hand-drawn chart while preserving Ovanan's real geography. Same overall silhouette — long narrow teardrop island oriented vertically, wider north end tapering to a point at the south. Western (left) shore: pale rocky cliff strip with chunky cartoon rock shapes. Eastern and central land: forest covered in many small poorly drawn cartoon trees. Small lawn patch as a lighter green smear near the buildings. Four to five crooked cartoon cottages in the north-central cluster. Two bent dock piers on the east shore. Teal cartoon water around the island.`
