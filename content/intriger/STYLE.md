...
- **Skatt kan betyda två saker** Agenten har en tendens att blanda ihop skatt (treasure) och skatt (tax). I den här filen och i `crews/` / `guests/` skriv **skatt** för både gömt föremål och avgift. Hitta inte på avgift, taxa eller klenod för att slippa ordet. Intern text skriver alltid `skatt (treasure)` och `skatt (tax)`: [`../huvudstory/skatt.md`](../huvudstory/skatt.md).
